...
class OrderBook extends React.Component {
      ...
      .subscribe((orders) => {
        ...
        if (bids.length) {
          bids.forEach((obj) => {
            ...
            this.setState({bids: obj.Quantity ? newBids.concat(obj) : newBids});
          });
        }
        ...
        if (asks.length) {
          asks.forEach((obj) => {
            ...
            this.setState({asks: obj.Quantity ? newAsks.concat(obj) : newAsks});
          });
        }
      });
  ...
  render() {
    ...
    const emptyRow = (<div className="emptyBookRow" style={rowStyle}>
      <div>-</div>
      <div>-</div>
      <div>-</div>
    </div>);
    ...
    const iconClassName = isAztec ? 'icon-cme-card-close icon--medium' : 'material-icons';
      ...
      <div>
        ...
        <div className="buttons-holder">
          <span id="centerBook" onClick={this.centerBookScroll}>
              <i className={isAztec ? 'fa fa-adjust' : 'material-icons'} aria-hidden="true"/>
            {AlphaPoint.translation('BUTTONS.TEXT_CENTER') || 'Center'}
          </span>
        </div>
        <div className="book-table-header">
          <div
            id="priceBookHeader">{`${AlphaPoint.translation('OPEN_ORDERS.PRICE_TEXT') || 'Price'} (${currentInstrument.Product2Symbol || 'USD'})`}</div>
          <div
            id="quantityBookHeader">{`${AlphaPoint.translation('OPEN_ORDERS.QUANTITY_TEXT') || 'Quantity'} (${currentInstrument.Product1Symbol || 'BTC'})`}</div>
          <div>{AlphaPoint.translation('OPEN_ORDERS.TITLE_TEXT') || 'Open Orders'}</div>
        </div>
        <div id="bookHolder">
          <div id="book" className="noselect">
            <div id="bookTable" className="booktable">
              ...
              <div id="bookSpread">
                {(spread > 0 ? spread : 0).toFixed(this.state.decimalPlaces)}${this.state.spreadSymbol}&nbsp;
