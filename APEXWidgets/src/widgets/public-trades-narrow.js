...
class Public_Trades_Narrow extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    const rows = rowsSlice
      ...
      .map(row => {
        ...
        if (row.Direction === 1) {
          direction = 'up';
          myStyle = { color: AlphaPoint.config.siteName === 'aztec' ? '#0079c1' : '#4662a1' };
        }
        if (row.Direction === 2) {
          direction = 'down';
          myStyle = { color: AlphaPoint.config.siteName === 'aztec' ? '#d60101' : '#af4141' };
        }
        // Note that this eventually must match pairs and show up or down tick
        return (
          <tr key={uuidV4()}>
            <td className={`borderless ${direction}-tick`} style={myStyle}>{row.Price.toFixed(this.state.decimalPlaces)}</td>
    ...
    return (
      <WidgetBase {...this.props} headerTitle={this.props.headerTitle || 'Trades'}>
...
export default Public_Trades_Narrow;
